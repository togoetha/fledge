...
spec:
  ...
  containers:
    - name: vkube-mgmt-service
      ...
      args: ["$ipaddr", "6443", "defaultPod.json"]
      ports:
        - name: http
          containerPort: 8180
      volumeMounts:
...
echo "--- Setup complete ---"
echo "Start a new kubelet with the following URL:"
echo "http://$ipaddr:8180/startVirtualKubelet?devName=<nodename>&serviceIP=<remoteIP>&servicePort=<remotePort>&kubeletPort=<remotePort2>"
