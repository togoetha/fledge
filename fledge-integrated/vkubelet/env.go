...
loop:
	for _, env := range container.Env {
		...
		case env.ValueFrom != nil && env.ValueFrom.FieldRef != nil:
			...
			// TG solution waiting for vkubelet team to do something about this
			res[env.Name] = GetReflectValue(pod, env.ValueFrom.FieldRef.FieldPath)
			continue loop
...
func GetReflectValue(pod interface{}, path string) string {
	fmt.Printf("Finding path %s in object\n", path)
	pathParts := strings.Split(path, ".")

	value := reflect.ValueOf(pod).Elem()
	fmt.Printf("Object value elem %s\n", value)
	for _, fieldName := range pathParts {
		fmt.Printf("Finding field %s\n", strings.Title(fieldName))
		field := value.FieldByName(strings.Title(fieldName))
		value = field
		fmt.Printf("Object value elem %s\n", value)
	}
	return value.String()
