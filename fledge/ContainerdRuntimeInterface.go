...
func (dri *ContainerdRuntimeInterface) DeployContainer(namespace string, pod *v1.Pod, dc *v1.Container) (string, error) {
	...
	//remove the damn container just to be sure everything is gone
	/*fmt.Println("Removing container first")
	oldContainers, err := dri.client.Containers(ctx)
	if len(oldContainers) > 0 {
		fmt.Println("Removing " + oldContainers[0].ID())
		oldContainers[0].Delete(ctx, containerd.WithSnapshotCleanup)
	}
	fmt.Println("REMOVING SNAPSHOT TOO EGADS CONTAINERD GET YOUR SHIT STRAIGHT")
	dri.client.SnapshotService(containerd.DefaultSnapshotter).Remove(ctx, snapshot)*/

	//json, _ := json.Marshal(specOpts)
...
//TODO: refactor this with the almost identical version in DockerRuntimeInterface
...
func (dri *ContainerdRuntimeInterface) FetchContainerLogs(namespace string, podName string, containerName string, tail string, timestamps bool) *io.ReadCloser {
	//TODO, need to handle file logging while creating containers, after that just read the file
	return nil
}
